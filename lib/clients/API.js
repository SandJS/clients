...
exports = module.exports = function(config) {
  class API extends Request {
    ...
    constructor(path, options) {
      options = _.merge({}, config.options, options || {});
      super(/^http/i.test(path) ? path : ((config.protocol || 'http') + '://' + config.host + ':' + config.port + Path.normalize('/' + config.pathPrefix + (/^\//.test(path) ? '' : '/') + path)), options);
