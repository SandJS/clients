...
exports = module.exports = function(config) {
  class API extends Request {
    ...
    constructor(path, options) {
      super(/^http/i.test(path) ? path : ((config.protocol || 'http') + '://' + config.host + ':' + config.port + Path.normalize('/' + config.pathPrefix + (/^\//.test(path) ? '' : '/') + path)), _.merge({}, config.options, options));
